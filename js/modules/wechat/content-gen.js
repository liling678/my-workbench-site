...
const IMG_GUARD = '。画面要求：真实相机拍摄的生活摄影照片，35mm胶片质感，自然柔和的窗边光线，浅景深背景虚化，构图留白，色调温暖柔和。不要3D渲染、不要插画、不要CG感、不要过度锐化和高饱和的塑料感。不出现人物手部和清晰人脸（如有人物仅远景背影或剪影），不出现手机电脑等电子屏幕，不出现任何文字字母，物体结构正确自然';
  ...
  el.querySelector('#autoCoverBtn').onclick = async () => {
    ...
    try {
      ...
      const planPrompt = [
        ...
        { role: 'user', content: `请阅读下面这篇公众号文章，为它规划 3~4 张配图（第一张必须是封面）。\n\n标题：${title}\n\n正文：\n${article.slice(0, 2000)}\n\n要求以 JSON 数组输出，每个元素形如：\n{"position":"封面/第一段后/第二段后/结尾","description":"一句话中文说明这张图的作用和画面","prompt":"用于AI绘画的详细画面描述，中文。必须描述成「真实相机拍摄的生活摄影照片」：具体到场景、主体、光线（自然光/窗边侧光/黄昏光）、色调（暖色、莫兰迪低饱和）、构图（留白、浅景深）、氛围。风格关键词：真实摄影、胶片感、生活气息、日系杂志感"}\n\nprompt 的硬性规则（AI绘画容易画崩画假，必须遵守）：\n1. 每个 prompt 都必须以「真实摄影照片，」开头，描述要像摄影师的拍摄脚本，不是插画设定\n2. 严禁出现人物的手部（拿书、端杯、托腮等任何手部动作都不行）\n3. 严禁出现清晰的人脸/五官；如需人物，只能是远景背影、剪影或虚化轮廓\n4. 优先选择：静物特写（摊开的书、冒热气的茶杯、台灯、信纸、毛毯）、窗边光影、自然风景、植物花草、空椅子、街角等无人场景\n5. 严禁出现手机、电脑等带屏幕的电子设备（屏幕朝向容易画错）\n6. 画面中不要出现任何文字、字母、数字\n7. 场景元素要少而精（1-2个主体足够），元素越多越容易画假\n\n只输出 JSON 数组本身。` },
      ];
